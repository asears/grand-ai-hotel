...
*A conductor in military green uniformΓÇöDEPUTY HENCKELSΓÇöapproaches and salutes.*
...
*They board. The train whistle soundsΓÇöa perfect C-sharp chordΓÇöand the locomotive begins its ascent.*
...
**M. GUSTAVE**  
Imagine, if you will, that you are hosting distinguished guestsΓÇölanguage models, in this case. When they arrive at your website or repository, how do they know where the dining room is? Where the library resides? Which documents are essential reading versus optional?
...
> A dense one-paragraph summaryΓÇöwhat it does, who it's for, key technologies.

Important context paragraphsΓÇöconstraints, architectural philosophy, non-obvious details.
...
**M. GUSTAVE**  
Codified at llmstxt.org. Think of it as the Baedeker guide for artificial intelligenceΓÇöconcise, hierarchical, actionable.
...
First: Exactly one H1 headingΓÇöthe project name. Required.

Second: A blockquote immediately followingΓÇödense, specific summary. Not generic boilerplate. Required.

Third: Context paragraphsΓÇönon-obvious constraints, architectural decisions, things an LLM might assume incorrectly. Highly recommended.

Fourth: H2 sections organizing documentation by topic. Each containing a list of links with one-sentence descriptions.

Fifth: A special "Optional" H2 sectionΓÇöits meaning explicitly defined. Links within can be skipped for token efficiency.
...
*The train enters a tunnel carved through solid rock. The compartment lightsΓÇöart deco brass fixturesΓÇöilluminate automatically.*
...
**AGATHA**  
This is for a hypothetical documentation websiteΓÇölet's call it "The Alpine Documentation Archive."
...
**LUDWIG**  
*(interjecting)*  
Critically: when inlining content, strip YAML frontmatter but preserve everything elseΓÇöMermaid diagrams, citations, tables, source comments.
...
**M. GUSTAVE**  
A skill teaches GitHub Copilot CLIΓÇöand its accompanying agentsΓÇöspecialized knowledge about your project. Think of it as... professional development for artificial intelligence.
...
- Quick Start Guide ΓåÆ 5-minute setup
- Engineer Onboarding ΓåÆ Complete orientation
- Safety Protocols ΓåÆ Mandatory pre-operation reading

### System Architecture (Priority: High)
- Brake Systems ΓåÆ Failure modes and redundancy
- Signal Communication ΓåÆ Station-to-train protocols
- Power Distribution ΓåÆ Voltage and safety margins

### Operational Guides (Priority: Medium)
- Daily Inspection ΓåÆ Pre-operation checklist
- Emergency Procedures ΓåÆ Critical incident response
- Maintenance Schedule ΓåÆ Recurring tasks
...
1. Check if topic is safety-critical ΓåÆ prioritize Getting Started and System Architecture
2. For operational questions ΓåÆ reference Operational Guides
3. For implementation ΓåÆ follow Quick Start Guide sequence
4. For research ΓåÆ mention Optional section explicitly
...
**Response:** "Start with the [Quick Start Guide](./getting-started/quickstart.md) which covers installation and local setup in under 5 minutes. Then review [Safety Protocols](./getting-started/safety.md)ΓÇömandatory reading before any operations."
...
*M. Gustave produces a small typewriter from beneath his seatΓÇöa perfectly proportioned portable model in cream with burgundy keys.*
...
*M. Gustave opens Copilot CLI on the typewriter displayΓÇösomehow it works despite being from 1932.*
...
### Path Strategy
- `./llms.txt` ΓåÆ Root discovery (GitHub MCP finds this first)
- `./wiki/llms.txt` ΓåÆ Wiki-relative links for VitePress
- `./wiki/llms-full.txt` ΓåÆ Complete inlined content
...
When answering questions:
- For quick lookups ΓåÆ Use root llms.txt
- For deep research ΓåÆ Reference llms-full.txt
- For website navigation ΓåÆ Use wiki/ paths
```
...
**M. GUSTAVE**  
Now we address delegationΓÇöusing skills with Copilot's subagent system.

**ZERO**  
Subagents, sir?

**M. GUSTAVE**  
When Copilot CLI encounters a complex task, it can spawn specialized subagentsΓÇöeach focused on a specific aspect. Skills enhance this process dramatically.
...
Main Agent (reads alpine-docs skill)
     Γöé
     Γö£ΓöÇΓöÇΓû║ Search Subagent
     Γöé    ΓööΓöÇ Finds relevant documentation using llms.txt structure
     Γöé
     Γö£ΓöÇΓöÇΓû║ Implementation Subagent
     Γöé    ΓööΓöÇ Follows Quick Start Guide sequence
     Γöé
     ΓööΓöÇΓöÇΓû║ Validation Subagent
          ΓööΓöÇ Checks safety protocols were reviewed
```
...
*AGATHA produces a small testing kit from her basketΓÇöperfectly arranged prompt cards.*
...
*The train approaches Sankt Moritz. A grand station appearsΓÇöwhite stone with art deco details, perfectly symmetrical. The locomotive slows with a melodious hiss of steam.*
...
```
project-root/
Γö£ΓöÇΓöÇ llms.txt          ΓåÉ Discovery file (root)
Γö£ΓöÇΓöÇ wiki/
Γöé   Γö£ΓöÇΓöÇ llms.txt      ΓåÉ Wiki-relative paths
Γöé   ΓööΓöÇΓöÇ llms-full.txt ΓåÉ Complete inlined content
```
...
export default {
  ...
  themeConfig: {
    nav: [
      ...
      { text: 'LLM Access', link: '/llms.txt' }  // ΓåÉ Link to llms.txt!
    ],
...
- `/en/` ΓåÆ English (default)
- `/de/` ΓåÆ German (Deutsch)
- `/fr/` ΓåÆ French (Fran├ºais)
- `/it/` ΓåÆ Italian (Italiano)
...
- `/v3/llms.txt` ΓåÆ Latest stable (3.2)
- `/v2/llms.txt` ΓåÆ Legacy support (2.5)
- `/v1/llms.txt` ΓåÆ Historical reference (1.8)
...
3. Combine both when appropriate:
   - "How does X work?" ΓåÆ Docs
   - "What is the current status of X?" ΓåÆ API
   - "Why is X behaving this way?" ΓåÆ Docs + API
```
...
Public repository:
- `/public/llms.txt` ΓåÆ Public-safe documentation
- Open source guides
- General architecture (sanitized)

Private repository:
- `/private/llms.txt` ΓåÆ Sensitive details
...
**M. GUSTAVE**  
Let me synthesize everything into a complete workflowΓÇöfrom empty repository to production-ready documented system with Copilot skills.
...
**AGATHA**  
Notice how each description is operationalΓÇöit tells you exactly what you'll learn and how long it takes.
...
3. Ensure skill file is in correct location
   ```
   .github/skills/
   ΓööΓöÇΓöÇ your-skill/
       ΓööΓöÇΓöÇ SKILL.md  ΓåÉ Must be named exactly "SKILL.md"
   ```
...
**ZERO**  
Thank you, Monsieur Gustave. I understand nowΓÇöllms.txt is the invitation, the skill is the cultural knowledge, and together they make our documentation truly welcoming.
...
*A train whistle soundsΓÇöthe return journey to Zurich.*

**M. GUSTAVE**  
Shall we, then? The valley awaits.

*They board. As the train begins its descent, the camera pulls back, revealing the complete symmetry of the station, the mountains, the railwayΓÇöeverything in perfect Wes Anderson composition.*
...
```
your-project/
Γö£ΓöÇΓöÇ llms.txt                     # Root discovery
Γö£ΓöÇΓöÇ .github/
Γöé   ΓööΓöÇΓöÇ skills/
Γöé       ΓööΓöÇΓöÇ your-docs/
Γöé           ΓööΓöÇΓöÇ SKILL.md         # Copilot skill
ΓööΓöÇΓöÇ wiki/
    Γö£ΓöÇΓöÇ llms.txt                 # Wiki-relative
    Γö£ΓöÇΓöÇ llms-full.txt            # Complete content
    ΓööΓöÇΓöÇ *.md                     # Your actual docs
```
...
## Usage Patterns
When user asks X ΓåÆ Reference Y
When user asks Z ΓåÆ Check A then B
...
if __name__ == '__main__':
    ...
    if issues:
        print("Γ¥î Validation failed:")
        for issue in issues:
            print(f"  - {issue}")
        exit(1)
    else:
        print("Γ£à llms.txt is valid")
```
...
**Tutorial Structure:**
- 4 acts mirroring the train journey (departure ΓåÆ ascent ΓåÆ summit ΓåÆ arrival)
...
Γòæ                                                                Γòæ
Γòæ  "Documentation is not merely information. It is an            Γòæ
Γòæ   invitationΓÇöto understanding, to contribution, to             Γòæ
Γòæ   excellence. Make it worthy of acceptance."                   Γòæ
Γòæ                                                                Γòæ
Γòæ                        ΓÇö M. Gustave H.                         Γòæ
Γòæ             Concierge, Grand Budapest Terminal                 Γòæ
Γòæ                                                                Γòæ
...
**Made with ≡ƒÜé and ≡ƒôÜ aboard The Alpine Express**
